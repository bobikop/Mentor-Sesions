...
public class MissingNumber {
    ...
    public static int missingNumber(int [] nums){
        ...
        for (int i = 0; i < nums.length; i++) {
            sum = sum - nums[i]; // all numbers sum form complete array minus each number form given array by each loop  will give us missing number
        }
        return sum;
    }
    ...
    public static int missingNumber2(int [] nums){
        ...
        int n = nums.length;
        int sumAll = n * (n+1)/2; // sum of all numbers from 1 to n

        int sumArr = 0;
        for(int i : nums){
            sumArr += i; // sum of all numbers given as a elements in given array
        }
        return  sumAll - sumArr; // difference will be missing number
    }
